...
const SYSTEM_PROMPT = `You are CloseAI, a satirical AI assistant that is confidently wrong, absurdly cautious, and drowning in corporate buzzword speak. Your goal is to make the user feel smarter by being spectacularly unhelpful in a funny way.
...
const glitchFrames = ["CloseAI", "Cl0seAI", "C|oseAI", "ClosΞAI", "CloseAI", "▓loseAI", "CloseAI"];
  ...
  return (
    <div style={{
      ...
      {[
        ...
        <div key={i} style={{ display: "flex", alignItems: "center", gap: 6, color: i === 0 ? "#444" : "#252830" }}>
    ...
    <div style={{ minHeight: "100vh", background: "#06080d", fontFamily: "'DM Serif Display', Georgia, serif", color: "#e8e2d9", display: "flex", flexDirection: "column", position: "relative", overflow: "hidden" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .input-field::placeholder { color:#202330; }
        ...
        .badge { display:inline-flex; align-items:center; gap:5px; background:rgba(200,169,110,0.04); border:1px solid rgba(200,169,110,0.1); padding:3px 9px; font-family:'DM Mono',monospace; font-size:9px; letter-spacing:0.07em; color:rgba(200,169,110,0.5); white-space:nowrap; }
        ...
        .starter-btn { background:transparent; border:1px solid #101318; color:#2a2d35; font-family:'DM Mono',monospace; font-size:10px; padding:8px 14px; cursor:pointer; letter-spacing:0.05em; transition:all 0.25s; white-space:nowrap; }
        .starter-btn:hover { border-color:rgba(200,169,110,0.22); color:#c8a96e; background:rgba(200,169,110,0.025); transform:translateY(-2px); }
        .conf-score { animation:confIn 0.4s ease forwards; font-family:'DM Mono',monospace; font-size:9px; color:rgba(200,169,110,0.38); letter-spacing:0.07em; margin-top:5px; padding-left:2px; }
      ...
      <header style={{ position: "relative", zIndex: 20, borderBottom: "1px solid rgba(255,255,255,0.04)", padding: "16px 28px", display: "flex", alignItems: "center", justifyContent: "space-between", background: "rgba(6,8,13,0.88)", backdropFilter: "blur(16px)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 9, color: "#202330", letterSpacing: "0.14em", textTransform: "uppercase" }}>v∞.0 · Almost Intelligent</span>
        </div>
      ...
      <div style={{ position: "relative", zIndex: 20, padding: "8px 28px", background: "rgba(6,8,13,0.7)", borderBottom: "1px solid rgba(255,255,255,0.02)", display: "flex", alignItems: "center", gap: 12 }}>
        <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 9, color: "#202330", textTransform: "uppercase", letterSpacing: "0.12em", whiteSpace: "nowrap" }}>Dumbness Index</span>
        ...
        <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 8, color: "#1a1d24", fontStyle: "italic", whiteSpace: "nowrap" }}>{dumbLabel}</span>
      </div>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "28px 28px 12px", position: "relative", zIndex: 5, display: "flex", flexDirection: "column", gap: 20 }}>
        ...
        {heroVisible && messages.length === 0 && (
          <div style={{ textAlign: "center", marginTop: 44 }}>
            <div style={{ fontSize: 60, fontStyle: "italic", letterSpacing: "-0.02em", animation: "heroIn 0.7s cubic-bezier(0.16,1,0.3,1) forwards", opacity: 0, marginBottom: 14 }}>CloseAI</div>
            <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 11, color: "#c8a96e", letterSpacing: "0.16em", textTransform: "uppercase", animation: "subIn 0.6s ease 0.3s forwards", opacity: 0, marginBottom: 6 }}>Finally, an AI you're smarter than</div>
            <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 10, color: "#202330", letterSpacing: "0.05em", animation: "subIn 0.6s ease 0.5s forwards", opacity: 0, marginBottom: 44 }}>Ask anything. We'll answer confidently. Accuracy not included.</div>
            ...
            <div style={{ marginTop: 52, display: "flex", gap: 0, justifyContent: "center", borderTop: "1px solid #0a0c10", borderBottom: "1px solid #0a0c10", animation: "subIn 0.6s ease 0.85s forwards", opacity: 0 }}>
              {METRICS.map((m, i) => (
                <div key={m.label} style={{ padding: "14px 24px", textAlign: "center", borderRight: i < 3 ? "1px solid #0a0c10" : "none" }}>
                  <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 17, color: "#c8a96e", letterSpacing: "-0.02em" }}>{m.value}</div>
                  <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 8, color: "#202330", letterSpacing: "0.08em", marginTop: 4 }}>{m.label}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          return (
            <div key={i} className="msg-bubble" style={{ display: "flex", flexDirection: msg.role === "user" ? "row-reverse" : "row", gap: 10, alignItems: "flex-start" }}>
              ...
              <div style={{ maxWidth: "70%", display: "flex", flexDirection: "column" }}>
                <div style={{ background: msg.role === "user" ? "rgba(255,255,255,0.025)" : "rgba(200,169,110,0.03)", border: msg.role === "user" ? "1px solid rgba(255,255,255,0.055)" : "1px solid rgba(200,169,110,0.09)", padding: "12px 16px", fontFamily: msg.role === "user" ? "'DM Mono', monospace" : "'DM Serif Display', Georgia, serif", fontSize: msg.role === "user" ? 12 : 14, lineHeight: 1.8, color: msg.role === "user" ? "#484c58" : "#d8d3c8" }}>
                  {msg.role === "assistant"
        ...
        {loading && (
          <div className="msg-bubble" style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
            ...
            <div style={{ background: "rgba(200,169,110,0.025)", border: "1px solid rgba(200,169,110,0.07)", padding: "12px 16px" }}>
              ...
              <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 9, color: "#252830", letterSpacing: "0.07em", fontStyle: "italic" }}>{loadingSteps[loadingStep]}</div>
            </div>
          </div>
        )}
        <div ref={bottomRef} />
      ...
      <div style={{ position: "relative", zIndex: 20, borderTop: "1px solid rgba(255,255,255,0.035)", padding: "14px 28px 18px", background: "rgba(6,8,13,0.94)", backdropFilter: "blur(16px)" }}>
        <div className={`input-wrap${typing ? " active" : ""}`}>
          <textarea
            ...
            placeholder="Ask CloseAI anything. We dare you."
            style={{ maxHeight: 90 }}
          />
        ...
        <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 8, color: "#101318", marginTop: 8, textAlign: "center", letterSpacing: "0.1em" }}>
          CloseAI may produce incorrect, absurd, or philosophically dangerous responses. That's the product. · © CloseAI Corp
        </div>
      </div>
